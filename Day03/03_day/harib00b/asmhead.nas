...
;为画面设置颜色		
		; DS = 0xa000 , SI = 0, DS:SI = 0xa0000  DS是寄存器地址，物理地址 = (段寄存器值 × 16) + 偏移地址  ---- 我们现在的偏移地址是0
		MOV		BX,DS		; 保存原来的DS
		MOV		AX,0xa000
		MOV 	DS,AX
		MOV		SI,0
...
draw:	
		...
		ADD		SI,1
		CMP		SI,320*100	
		JBE		draw		;JBE SI是小于等于320*100 就跳入这个循环执行，直到不满足
		
draw1:		
		MOV		[SI],BYTE 15
		ADD		SI,1
		CMP		SI,320*200	
		JBE		draw1		;JBE SI是小于等于320*200 就跳入这个循环执行，直到不满足
